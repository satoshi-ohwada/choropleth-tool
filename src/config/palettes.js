// Color Palettes Configuration & Hatching Patterns

export const PALETTES = {
  viridis: ["#440154", "#443983", "#31688e", "#21918c", "#35b779", "#8fd744", "#fde725"],
  cividis: ["#00204d", "#1f3c6d", "#415984", "#677598", "#9093a8", "#bfb5b3", "#ffe945"],
  blues: ["#ffffff", "#dbeafe", "#bfdbfe", "#60a5fa", "#2563eb", "#1e40af", "#172554"],
  greens: ["#ffffff", "#dcfce7", "#bbf7d0", "#4ade80", "#16a34a", "#15803d", "#052e16"],
  oranges: ["#ffffff", "#ffedd5", "#fed7aa", "#fb923c", "#ea580c", "#c2410c", "#431407"],
  reds: ["#ffffff", "#fee2e2", "#fca5a5", "#ef4444", "#dc2626", "#991b1b", "#450a0a"],
  purples: ["#ffffff", "#f3e8ff", "#e9d5ff", "#c084fc", "#9333ea", "#6b21a8", "#3b0764"],
  ylorrd: ["#ffffb2", "#fed976", "#feb24c", "#fd8d3c", "#fc4e2a", "#e31a1c", "#b10026"],
  grayscale: ["#ffffff", "#e2e8f0", "#cbd5e1", "#94a3b8", "#64748b", "#334155", "#0f172a"],
  spectral: ["#9e0142", "#d53e4f", "#f46d43", "#fdae61", "#fee08b", "#e6f598", "#abdda4", "#66c2a5", "#3288bd", "#5e4fa2"],
  plasma: ["#0d0887", "#46039f", "#7201a8", "#9c179e", "#bd3786", "#d8576b", "#ed7953"],
  magma: ["#000004", "#180f3d", "#440f76", "#721f81", "#9e2f7f", "#cd4071", "#f1605d"],
  paired: ["#a6cee3", "#1f78b4", "#b2df8a", "#33a02c", "#fb9a99", "#e31a1c", "#fdbf6f", "#ff7f00", "#cab2d6", "#6a3d9a", "#ffff99", "#b15928"],
  set1: ["#e41a1c", "#377eb8", "#4daf4a", "#984ea3", "#ff7f00", "#ffff33", "#a65628", "#f781bf"],
  // Dedicated Z-score / Diverging Palettes
  div_blue_red: ["#2166ac", "#67a9cf", "#d1e5f0", "#f7f7f7", "#fddbc7", "#ef8a62", "#b2182b"],
  div_red_blue: ["#b2182b", "#ef8a62", "#fddbc7", "#f7f7f7", "#d1e5f0", "#67a9cf", "#2166ac"],
  div_orange_purple: ["#b35806", "#f1a340", "#fee0b6", "#f7f7f7", "#d8daeb", "#998ec3", "#542788"],
  div_brown_teal: ["#8c510a", "#d8b365", "#f6e8c3", "#f5f5f5", "#c7eae5", "#5ab4ac", "#01665e"],
  div_spectral: ["#d53e4f", "#fdae61", "#fee08b", "#ffffbf", "#e6f598", "#abdda4", "#2b83ba"],
  patterns: ["url(#pat-0)", "url(#pat-1)", "url(#pat-2)", "url(#pat-3)", "url(#pat-4)", "url(#pat-5)", "url(#pat-6)", "url(#pat-7)"]
};
